...
class ImportJob:
    ...
    def model_class(self):
        if self._model_class is None:
            self._model_class = get_model_class(self.model)
...
class ImportOperation:
    def __init__(self, job_id: str, jobs_dir: str, repo: Repository, ssh_private_key: str = None):
        self.job = ImportJob(job_id=job_id, jobs_dir=jobs_dir)
        logger.debug(f'Job directory is {self.job.dir}')
        self.repo = repo
    ...
    def resume(self, **kwargs):
        if not self.job.dir_exists:
            raise RuntimeError(f'Cannot resume job "{self.job.id}": no such job directory: "{self.job.dir}"')

        logger.info(f'Resuming saved job {self.job.id}')
        try:
            # load stored config from the previous run of this job
            self.job.load_config()
        except FileNotFoundError:
            raise RuntimeError(f'Cannot resume job {self.job.id}: no config.yml found in {self.job.dir}')

        return (yield from self.import_items(**kwargs))

    def import_items(self, limit: int = None, percentage=None, validate_only: bool = False, import_file=None):
        ...
        initial_completed_item_count = len(self.job.completed_log)
        logger.info(f'Found {initial_completed_item_count} completed items')

        import_run = self.job.new_run().start()
        for row in metadata:
            ...
            missing_files = [
                name for name in row.filenames if not self.get_source(self.job.binaries_location, name).exists()
            ]
    ...
    def update_repo(self, metadata, row, repo_changeset):
        """
        ...
        if not item.created:
            ...
            if self.job.access is not None:
                item.rdf_type.append(URIRef(self.job.access))
            # add the collection membership
            if self.job.member_of is not None:
                item.member_of = URIRef(self.job.member_of)
            ...
            if row.has_files:
                ...
                self.add_files(
                    ...
                    base_location=self.job.binaries_location,
                    access=self.job.access,
            ...
            try:
                with self.repo.client.transaction() as txn_client:
                    item.create(txn_client, container_path=self.job.container)
                    item.update(txn_client)
                    txn_client.commit()
            except Exception as e:
                raise RuntimeError(f'Creating item failed: {e}') from e

            self.job.complete(item, row.line_reference, ImportedItemStatus.CREATED)
            metadata.created += 1
            ...
            self.job.complete(item, row.line_reference, ImportedItemStatus.MODIFIED)
            metadata.updated += 1
            updated_uris.append(item.uri)

        else:
            self.job.complete(item, row.line_reference, ImportedItemStatus.UNCHANGED)
